...
func BaseAccountToDecAccount(account auth.BaseAccount) DecAccount {
	...
	for _, coin := range account.Coins {
		dec := coin.Amount
		decCoin := sdk.NewDecCoinFromDec(coin.Denom, dec.ToDec())
...
func (acc *DecAccount) ToBaseAccount() *auth.BaseAccount {
	display_coin := utils.ParseDecCoinRounded(acc.Coins)
...
func DecAccountArrToBaseAccountArr(decAccounts []DecAccount) (baseAccountArr []authexported.Account) {
