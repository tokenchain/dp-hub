...
func getMockDexApp(t *testing.T, numGenAccs int) (mockDexApp *MockDexApp, keeper Keeper, addrs []sdk.AccAddress) {
	...
	mockDexApp.tokenKeeper = keeper.NewKeeper(
	...
	handler := token.NewTokenHandler(mockDexApp.tokenKeeper, version.CurrentProtocolVersion)

	mockDexApp.Router().AddRoute(token.RouterKey, handler)
	mockDexApp.QueryRouter().AddRoute(token.QuerierRoute, token.NewQuerier(mockDexApp.tokenKeeper))
...
ok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-bok-b`,
	...
	require.NotNil(t, token.handleMsgTokenIssue(ctx, keeper, MsgErrorSupply, nil))
	...
	keeper.AddTokenSuffix(ctx, keeper, "notexist")
...
func TestCreateTokenMint(t *testing.T) {
	...
	coins := utils.MustParseCoins(btcTokenSymbol, "2000")
	//coins = append(coins, sdk.MustParseCoins(common.NativeToken, "1.0375")...)
	coins = append(coins, utils.MustParseCoins(common.NativeToken, "1.0")...)
	coins = append(coins, utils.MustParseCoins(xmrTokenSymbol, "1000")...)
...
func TestCreateMsgTokenSend(t *testing.T) {
	...
	for _, acc := range accounts {
		if acc.GetAddress().Equals(testAccounts[0].baseAccount.Address) {
			senderCoins := utils.MustParseCoins(tokenName, "900")
			senderCoins = append(senderCoins, utils.MustParseCoins(common.NativeToken, "97500")...)
			require.EqualValues(t, senderCoins, acc.GetCoins())
		} else if acc.GetAddress().Equals(testAccounts[1].baseAccount.Address) {
			receiverCoins := utils.MustParseCoins(tokenName, "100")
			receiverCoins = append(receiverCoins, utils.MustParseCoins(common.NativeToken, "100000")...)
	...
	for _, acc := range accounts {
		if acc.GetAddress().Equals(testAccounts[0].baseAccount.Address) {
			senderCoins := utils.MustParseCoins(tokenName, "800")
			senderCoins = append(senderCoins, utils.MustParseCoins(common.NativeToken, "97400")...)
			require.EqualValues(t, senderCoins.String(), acc.GetCoins().String())
		} else if acc.GetAddress().Equals(testAccounts[1].baseAccount.Address) {
			receiverCoins := utils.MustParseCoins(tokenName, "200")
			receiverCoins = append(receiverCoins, utils.MustParseCoins(common.NativeToken, "100100")...)
			require.EqualValues(t, receiverCoins, acc.GetCoins())
		}
	}
}
...
func TestCreateMsgMultiSend(t *testing.T) {
	...
	for _, acc := range accounts {
		if acc.GetAddress().Equals(testAccounts[0].baseAccount.Address) {
			...
			senderCoins := utils.MustParseCoins(btcSymbol, "998")

			senderCoins = append(senderCoins, utils.MustParseCoins(common.NativeToken, "97498.97000000")...)
			require.EqualValues(t, senderCoins, acc.GetCoins())
		} else if acc.GetAddress().Equals(testAccounts[1].baseAccount.Address) {
			receiverCoins := utils.MustParseCoins(btcSymbol, "2")
			receiverCoins = append(receiverCoins, utils.MustParseCoins(common.NativeToken, "100001")...)
			require.EqualValues(t, receiverCoins, acc.GetCoins())
		}
	}
}
