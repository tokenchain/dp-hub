...
func execRule3(book *types.DepthBook, offset int, refPrice sdk.Dec, pricePrecision int64,
	...
	if imbalance[indexesRule2[0]-offset].GT(sdk.ZeroDec()) {
		...
		newRefPrice = sdk.NewDecFromBigIntWithPrec(newRefPrice.Int, pricePrecision)
		...
		newRefPrice := sdk.NewDecFromBigIntWithPrec(refPrice.Int, pricePrecision)
		bestPrice = bestPriceFromRefPrice(book.Items[indexesRule2[0]].Price,
			book.Items[indexesRule2[indexLen2-1]].Price, newRefPrice)
	}
...
func executeMatchedUpdatedProduct(ctx sdk.Context, k keeper.Keeper,
	...
	deals, blockRemainDeals := fillDepthBook(ctx, k, product, matchResult.Price, matchResult.Quantity, &buyExecutedCnt, &sellExecutedCnt, blockRemainDeals, feeParams)
	matchResult.Deals = deals
	updatedProductsBasePrice[product] = matchResult
	// info sprint
	logger.Info(fmt.Sprintf(
		"matchResult(%d-%s): price: %v, quantity: %v, buyExecuted: %v, sellExecuted: %v, dealsNum: %d",
		matchResult.BlockHeight, product,
		matchResult.Price, matchResult.Quantity,
		buyExecutedCnt, sellExecutedCnt, len(deals)))
...
func executeLockedProduct(ctx sdk.Context, k keeper.Keeper,
	...
	deals, blockRemainDeals := fillDepthBook(ctx, k, product, lock.Price, lock.Quantity, &buyExecuted, &sellExecuted, blockRemainDeals, feeParams)
	...
	logger.Info(fmt.Sprintf(
		"BlockHeight<%d> execute locked product(%s<%d>): price: %v, quantity: %v, buyExecuted: %v, sellExecuted: %v, dealsNum: %d",
	...
	if buyExecuted.GTE(lock.Quantity) && sellExecuted.GTE(lock.Quantity) {
		...
		logger.Info(fmt.Sprintf(
			"BlockHeight<%d> unlock product(%s<%d>)",
			blockHeight,
			product,
			lock.BlockHeight))
	} else {
