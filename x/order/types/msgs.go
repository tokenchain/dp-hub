...
func (msg MsgNewOrders) ValidateBasic() error {
	if msg.Sender.Empty() {
		return did.ErrInvalidAddress(msg.Sender.String())
	}
	if msg.OrderItems == nil || len(msg.OrderItems) == 0 {
		return did.UnknownRequest("invalid OrderItems")
	}
	if len(msg.OrderItems) > OrderItemLimit {
		return did.UnknownRequest("Numbers of NewOrderItem should not be more than " + strconv.Itoa(OrderItemLimit))
	}
	for _, item := range msg.OrderItems {
		if len(item.Product) == 0 {
			return did.UnknownRequest("Product cannot be empty")
		}
		symbols := strings.Split(item.Product, "_")
		if len(symbols) != 2 {
			return did.UnknownRequest("Product should be in the format of \"base_quote\"")
		}
		if symbols[0] == symbols[1] {
			return did.UnknownRequest("invalid product")
		}
		if item.Side != BuyOrder && item.Side != SellOrder {
			return did.UnknownRequest(
				fmt.Sprintf("Side is expected to be \"BUY\" or \"SELL\", but got \"%s\"", item.Side))
		}
		if !(item.Price.IsPositive() && item.Quantity.IsPositive()) {
			return did.UnknownRequest("Price/Quantity must be positive")
		}
	}
...
func (msg MsgCancelOrders) ValidateBasic() error {
	...
	if msg.OrderIDs == nil || len(msg.OrderIDs) == 0 {
		return did.UnknownRequest("invalid OrderIDs")
	}
	if len(msg.OrderIDs) > MultiCancelOrderItemLimit {
		return did.UnknownRequest("Numbers of CancelOrderItem should not be more than " + strconv.Itoa(OrderItemLimit))
	}
	if hasDuplicatedID(msg.OrderIDs) {
		return did.UnknownRequest("Duplicated order ids detected")
	}
	for _, item := range msg.OrderIDs {
		if item == "" {
			return did.Unauthorized("orderID cannot be empty")
		}
	}
...
type OrderResult struct {
	Code    did.CodeType `json:"code"`    // order return code
