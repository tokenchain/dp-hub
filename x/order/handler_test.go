...
import (
	...
	tokentypes "github.com/tokenchain/ixo-blockchain/x/token/types"
)
...
func TestHandleMsgMultiCancelOrder(t *testing.T) {
	...
	orderIDsMap := keeper.GetDiskCache().GetOrderIDsMapCopy()
	require.Equal(t, 1, len(orderIDsMap.Data))
	require.Equal(t, types.FormatOrderID(10, 1), orderIDsMap.Data[types.FormatOrderIDsKey(order.Product, order.Price, order.Side)][0])
	require.Equal(t, 1, len(keeper.GetDiskCache().GetUpdatedOrderIDKeys()))
...
func TestValidateMsgMultiNewOrder(t *testing.T) {
	...
	result := ValidateMsgNewOrders(ctx, keeper, msg)
	require.EqualValues(t, did.CodeOK, result)
...
func handleOrders(t *testing.T, baseasset string, quoteasset string, orders []*types.Order, blockheight int64) sdk.Coins {
	...
	coins, err := sdk.ParseDecCoins(fmt.Sprintf("%d%s,%d%s", 100, baseasset, 100, quoteasset))
	decCoins := utils.ParseDecCoinRounded(coins)
	require.Nil(t, err)
