...
func checkOrderNewMsg(ctx sdk.Context, keeper keeper.Keeper, msg types.MsgNewOrder) error {
	...
	roundedPrice := sdk.NewDecFromBigIntWithPrec(msg.Price.Int, priceDigit)
	roundedQuantity := sdk.NewDecFromBigIntWithPrec(msg.Quantity.Int, quantityDigit)
...
func handleNewOrder(ctx sdk.Context, k Keeper, sender sdk.AccAddress,
	...
	if err != nil {
		...
		if k.IsProductLocked(ctx, msg.Product) {
			...
			code = did.CodeInsufficientCoins
		}
	}
...
func handleMsgNewOrders(ctx sdk.Context, k Keeper, msg types.MsgNewOrders,
	...
	if handlerResult.None() {
		return &sdk.Result{}, did.IntErr("no result")
	}
...
func ValidateMsgNewOrders(ctx sdk.Context, k keeper.Keeper, msg types.MsgNewOrders) error {
	ratio := "1"
	if len(msg.OrderItems) > 1 {
		ratio = "0.8"
	}
	...
	for _, item := range msg.OrderItems {
		...
		if err != nil {
			return did.UnknownRequestErr("code unknown", err)
		}
		if k.IsProductLocked(ctx, msg.Product) {
			return did.IntErr(err.Error())
		}
		order := getOrderFromMsg(ctx, k, msg, ratio)
		_, err = k.TryPlaceOrder(ctx, order)
		if err != nil {
			return did.InsufficientCoins(err.Error())
		}
	}
	return nil
}
...
func handleCancelOrder(context sdk.Context, k Keeper, sender sdk.AccAddress, orderID string, logger log.Logger) (
	...
	code, validateResult := validateCancelOrder(ctx, k, msg)
	var message string
...
func handleMsgCancelOrders(ctx sdk.Context, k Keeper, msg types.MsgCancelOrders, logger log.Logger) (*sdk.Result, error) {
	...
	if handlerResult.None() {
		return &sdk.Result{}, did.IntErr("internal code")
	}
...
func validateCancelOrder(ctx sdk.Context, keeper keeper.Keeper, msg types.MsgCancelOrder) (uint32, error) {
	order := keeper.GetOrder(ctx, msg.OrderID)
	// Check order
	if order == nil {
		return 6, did.UnknownRequest(fmt.Sprintf("order(%s) does not exist or already closed", msg.OrderID))
	}
	if order.Status != types.OrderStatusOpen {
		return 111222, did.IntErr(fmt.Sprintf("cannot cancel order with status(%d)", order.Status))
	}
	if !order.Sender.Equals(msg.Sender) {
		return 4, did.Unauthorized(fmt.Sprintf("not the owner of order(%v)", msg.OrderID))
	}
	if keeper.IsProductLocked(ctx, order.Product) {
		return 111222, did.IntErr(fmt.Sprintf("the trading pair (%s) is locked, please retry later", order.Product))
	}
	return 0, nil
}
...
func ValidateMsgCancelOrders(ctx sdk.Context, keeper keeper.Keeper, msg types.MsgCancelOrders) error {
	for _, orderID := range msg.OrderIDs {
		...
		code, res := validateCancelOrder(ctx, keeper, msg)
		if 0 != code {
			return res
		}
	}
