...
func (k Keeper) GetBond(ctx sdk.Context, bondDid types2.Did) (bond types.Bond, found bool) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetBondDid(ctx sdk.Context, bondToken string) (bondDid types2.Did, found bool) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) MustGetBond(ctx sdk.Context, bondDid types2.Did) types.Bond {
...
func (k Keeper) BondExists(ctx sdk.Context, bondDid types2.Did) bool {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) SetBond(ctx sdk.Context, bondDid types2.Did, bond types.Bond) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetBondKey(bondDid), k.cdc.MustMarshalBinaryBare(bond))
}

func (k Keeper) SetBondDid(ctx sdk.Context, bondToken string, bondDid types2.Did) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetBondDidsKey(bondToken), k.cdc.MustMarshalBinaryBare(bondDid))
}

func (k Keeper) GetReserveBalances(ctx sdk.Context, bondDid types2.Did) sdk.Coins {
	bond := k.MustGetBond(ctx, bondDid)
	return k.BankKeeper.GetCoins(ctx, bond.ReserveAddress)
}

func (k Keeper) GetSupplyAdjustedForBuy(ctx sdk.Context, bondDid types2.Did) sdk.Coin {
	bond := k.MustGetBond(ctx, bondDid)
...
func (k Keeper) GetSupplyAdjustedForSell(ctx sdk.Context, bondDid types2.Did) sdk.Coin {
	bond := k.MustGetBond(ctx, bondDid)
	batch := k.MustGetBatch(ctx, bondDid)
	supply := bond.CurrentSupply
...
func (k Keeper) SetCurrentSupply(ctx sdk.Context, bondDid types2.Did, currentSupply sdk.Coin) {
