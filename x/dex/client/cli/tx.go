...
import (
	...
	cutils "github.com/tokenchain/ixo-blockchain/client/utils"
...
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if len(baseAsset) == 0 {
				return ErrCli("failed. empty base asset")
			}
...
func getMultiSignsCmd(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(stdTx.Msgs) == 0 {
				return ErrCli("msg is empty")
			}

			msg, ok := stdTx.Msgs[0].(types.MsgTransferOwnership)
			if !ok {
				return ErrCli("invalid msg type")
			}
...
`, version.ClientName, sdk.DefaultBondDenom, sdk.DefaultBondDenom, sdk.DefaultBondDenom,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			proposal, err := cutils.ParseDelistProposalJSON(cdc, args[0])
			if err != nil {
				return err
			}
			...
			msg := gov.NewMsgSubmitProposal(content, cutils.ParseDecCoinRounded(proposal.Deposit), from)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg})
		},
	}
...
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				return did.ErrInvalidAddress(fmt.Sprintf("invalid address：%s", feeAddrStr))
			}
			owner := cliCtx.GetFromAddress()
