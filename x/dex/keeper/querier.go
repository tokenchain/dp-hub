...
func NewQuerier(keeper IKeeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		default:
			return nil, did.UnknownRequest("unknown dex query endpoint")
		}
	}
}
...
func queryProduct(ctx sdk.Context, req abci.RequestQuery, keeper IKeeper) (res []byte, err error) {
	...
	if errUnmarshal != nil {
		return nil, did.UnknownRequestErr("incorrectly formatted request data", errUnmarshal)
	}

	offset, limit := common.GetPage(int(params.Page), int(params.PerPage))

	if offset < 0 || limit <= 0 {
		return nil, did.UnknownRequest(fmt.Sprintf("invalid params: page=%d or per_page=%d", params.Page, params.PerPage))
	}
	...
	if params.Owner != "" {
		...
		if err != nil {
			return nil, did.InvalidAddress(fmt.Sprintf("invalid address：%s", params.Owner))
		}
	...
	if errMarshal != nil {
		return nil, did.IntErr("failed to  marshal result to JSON")
	}
	return res, nil
...
func queryDeposits(ctx sdk.Context, req abci.RequestQuery, keeper IKeeper) (res []byte, err error) {
	...
	if params.Address == "" && params.BaseAsset == "" && params.QuoteAsset == "" {
		return nil, did.UnknownRequest("bad request: address、base_asset and quote_asset could not be empty at the same time")
	}
	...
	for i, tokenPair := range tokenPairs {
		if tokenPair == nil {
			return nil, did.IntErr("unexpected token pair")
		}
		// filter address
	...
	res, errMarshal := json.MarshalIndent(response, "", "  ")
	if errMarshal != nil {
		return nil, did.IntErr(errMarshal.Error())
	}
...
func queryParams(ctx sdk.Context, _ abci.RequestQuery, keeper IKeeper) (res []byte, err error) {
	...
	if errUnmarshal != nil {
		return nil, did.IntErr(errUnmarshal.Error())
	}
	return res, nil
}
...
// nolint
func queryOperator(ctx sdk.Context, req abci.RequestQuery, keeper IKeeper) ([]byte, error) {
	var params types.QueryDexOperatorParams
	err := types.ModuleCdc.UnmarshalJSON(req.Data, &params)
	if err != nil {
		return nil, did.UnknownRequestErr("incorrectly formatted request data", err)
	}
	...
	if err != nil {
		return nil, did.IntErr(err.Error())
	}
	return bz, nil
}

// nolint
func queryOperators(ctx sdk.Context, keeper IKeeper) ([]byte, error) {
