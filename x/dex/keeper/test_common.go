...
func createTestInputWithBalance(t *testing.T, numAddrs, initQuantity int64) testInput {
	...
	accountKeeper := auth.NewAccountKeeper(cdc, keyAcc,
		paramsKeeper.Keeper.Subspace(auth.DefaultParamspace), auth.ProtoBaseAccount)
	bankKeeper := bank.NewBaseKeeper(
		accountKeeper,
		paramsKeeper.Keeper.Subspace(bank.DefaultParamspace),
		blacklistedAddrs,
	)
	...
	// token keeper
	tokenKeepr := keeper.NewKeeper(bankKeeper,
		paramsKeeper.Keeper.Subspace(token.DefaultParamspace), auth.FeeCollectorName, supplyKeeper,
		keyToken, keyLock, cdc, true)

	paramsSubspace := paramsKeeper.Keeper.Subspace(types.DefaultParamspace)
	...
	for i := int64(0); i < numAddrs; i++ {
		...
		coins := utils.ParseDecCoinRounded(decCoins)
		err := dexKeeper.supplyKeeper.MintCoins(ctx, token.ModuleName, coins)
		require.Nil(t, err)
		err = dexKeeper.supplyKeeper.SendCoinsFromModuleToAccount(ctx, token.ModuleName, addr, coins)
		require.Nil(t, err)
	}
...
func (keeper mockBankKeeper) GetCoins(ctx sdk.Context, addr sdk.AccAddress) sdk.Coins {
	return sdk.Coins{sdk.NewCoin(common.NativeToken, sdk.NewInt(2500))}
}
