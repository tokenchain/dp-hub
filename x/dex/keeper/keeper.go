...
import (
	...
	"github.com/tokenchain/ixo-blockchain/x/stream/exported"
...
func (k Keeper) Deposit(ctx sdk.Context, product string, from sdk.AccAddress, amount sdk.DecCoin) error{
	...
	if tokenPair == nil {
		return did.UnknownRequest(fmt.Sprintf("failed to deposit because non-exist product: %s", product))
	}

	if !tokenPair.Owner.Equals(from) {
		return did.InvalidAddress(fmt.Sprintf("failed to deposit because %s is not the owner of product:%s", from.String(), product))
	}

	if amount.Denom != sdk.DefaultBondDenom {
		return did.UnknownRequest(fmt.Sprintf("failed to deposit because deposits only support %s token", sdk.DefaultBondDenom))
	}


	depositCoin := utils.ListParsedCoinRounded(amount)
	err := k.GetSupplyKeeper().SendCoinsFromAccountToModule(ctx, from, types.ModuleName, depositCoin)
	if err != nil {
		return did.InsufficientCoins(fmt.Sprintf("failed to deposits because insufficient deposit coins(need %s)", depositCoin.String()))
	}
...
func (k Keeper) Withdraw(ctx sdk.Context, product string, to sdk.AccAddress, amount sdk.DecCoin) error {
	tokenPair := k.GetTokenPair(ctx, product)
	if tokenPair == nil {
		return did.UnknownRequest(fmt.Sprintf("failed to withdraws because non-exist product: %s", product))
	}

	if !tokenPair.Owner.Equals(to) {
		return  did.InvalidAddress(fmt.Sprintf("failed to withdraws because %s is not the owner of product:%s", to.String(), product))
	}

	if amount.Denom != sdk.DefaultBondDenom {
		return did.UnknownRequest(fmt.Sprintf("failed to withdraws because deposits only support %s token", sdk.DefaultBondDenom))
	}

	if tokenPair.Deposits.IsLT(amount) {
		return did.InsufficientCoins(fmt.Sprintf("failed to withdraws because deposits:%s is less than withdraw:%s", tokenPair.Deposits.String(), amount.String()))
	}
...
func (k Keeper) TransferOwnership(ctx sdk.Context, product string, from sdk.AccAddress, to sdk.AccAddress) error {
	...
	if !tokenPair.Owner.Equals(from) {
		return did.Unauthorized(fmt.Sprintf("%s is not the owner of product(%s)", from.String(), product))
	}

	// Withdraw
	if tokenPair.Deposits.IsPositive() {
		if err := k.Withdraw(ctx, product, from, tokenPair.Deposits); err != nil {
			return did.IntErr(fmt.Sprintf("withdraw deposits:%s error:%s", tokenPair.Deposits.String(), err.Error()))
		}
	}
