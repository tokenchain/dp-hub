...
import (
	...
	ordertypes "github.com/tokenchain/ixo-blockchain/x/order/types"
)
...
func (k *mockSupplyKeeper) behave() error {
	if k.behaveEvil {
		return did.IntErr("raise an mock exception here")
	}
	return nil
}
...
func (k *mockSupplyKeeper) MintCoins(ctx sdk.Context, moduleName string, amt sdk.Coins) error {
	return k.behave()
}
...
func newMockApp(tokenKeeper TokenKeeper, supplyKeeper SupplyKeeper, accountsInGenisis int) (
	...
	genAccs, _, _, _ := mock.CreateGenAccounts(accountsInGenisis, utils2.ParseDecCoinRounded(decCoins))
