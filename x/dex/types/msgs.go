...
func (msg MsgList) ValidateBasic() error {
	if msg.ListAsset == msg.QuoteAsset {
		return did.ErrInvalidCoins("failed to list product because base asset is same as quote asset")
	}

	if !msg.InitPrice.IsPositive() {
		return did.UnknownRequest("invalid init price")
	}

	if msg.Owner.Empty() {
		return did.ErrInvalidAddress("missing owner address")
	}
	return nil
}
...
func (msg MsgDeposit) ValidateBasic() error {
	if msg.Depositor.Empty() {
		return did.ErrInvalidAddress(msg.Depositor.String())
	}
	if !msg.Amount.IsValid() || !msg.Amount.IsPositive() {
		return did.ErrInvalidCoins(msg.Amount.String())
	}
...
func (msg MsgTransferOwnership) ValidateBasic() error {
	if msg.FromAddress.Empty() {
		return did.ErrInvalidAddress("missing sender address")
	}

	if msg.ToAddress.Empty() {
		return did.ErrInvalidAddress("missing recipient address")
	}
	...
	if !msg.checkMultiSign() {
		return did.Unauthorized("invalid multi signature")
	}
	return nil
}
...
func (msg MsgCreateOperator) ValidateBasic() error {
	if msg.Owner.Empty() {
		return did.InvalidAddress("missing owner address")
	}
	if msg.HandlingFeeAddress.Empty() {
		return did.InvalidAddress("missing handling fee address")
	}
