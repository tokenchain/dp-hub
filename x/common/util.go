...
func HasSufficientCoins(addr sdk.AccAddress, availableCoins, amt sdk.Coins) (err error) {
	...
	if !amt.IsValid() {
		return did.ErrInvalidCoins(amt.String())
	}
	if !availableCoins.IsValid() {
		return did.ErrInvalidCoins(availableCoins.String())
	}

	_, hasNeg := availableCoins.SafeSub(amt)
	if hasNeg {
		return did.InsufficientCoins(
...
func HasSufficientDecCoins(addr sdk.AccAddress, availableCoins, amt sdk.DecCoins) (err error) {
	//availableCoins := availCoins[:]
	if !amt.IsValid() {
		return did.ErrInvalidCoins(amt.String())
	}
