...
func NewRestServer(cdc *codec.Codec) *RestServer {
	...
	logger := log.NewTMLogger(log.NewSyncWriter(os.Stdout)).With("module", "fc-rest-server")
...
func ServeCommand(cdc *codec.Codec, registerRoutesFn func(*RestServer)) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rest-server",
		Short: "Start DP Faucet (light-client daemon), a local REST server",
...
func (rs *RestServer) registerSwaggerUI() {
	...
	rs.Mux.PathPrefix("/fc-swagger-ui/").Handler(http.StripPrefix("/swagger-ui/", staticServer))
}
...
func (rs *RestServer) Start(listenAddr string, maxOpen int, readTimeout, writeTimeout uint) (err error) {
	...
	rs.log.Info(
		fmt.Sprintf(
			"Starting faucet application REST service (chain-id: %q)...",
