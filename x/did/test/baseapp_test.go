...
import (
	...
	appbase "github.com/cosmos/cosmos-sdk/baseapp"
...
func newBaseApp(name string, options ...func(*appbase.BaseApp)) *appbase.BaseApp {
	logger := defaultLogger()
	db := dbm.NewMemDB()
	codec := codec.New()
	registerTestCodec(codec)
	return appbase.NewBaseApp(name, logger, db, testTxDecoder(codec), options...)
}
...
func setupBaseApp(t *testing.T, options ...func(*appbase.BaseApp)) *appbase.BaseApp {
...
func TestLoadVersion(t *testing.T) {
	logger := defaultLogger()
	pruningOpt := appbase.SetPruning(store.PruneNothing)
	db := dbm.NewMemDB()
	name := t.Name()
	app := appbase.NewBaseApp(name, logger, db, nil, pruningOpt)

	// make a cap key and mount the store
	capKey := sdk.NewKVStoreKey(appbase.MainStoreKey)
...
func useDefaultLoader(app *appbase.BaseApp) {
	app.SetStoreLoader(appbase.DefaultStoreLoader)
}

func useUpgradeLoader(upgrades *store.StoreUpgrades) func(*appbase.BaseApp) {
	return func(app *appbase.BaseApp) {
		app.SetStoreLoader(appbase.StoreLoaderWithUpgrade(upgrades))
	}
}

func useFileUpgradeLoader(upgradeInfoPath string) func(*appbase.BaseApp) {
	return func(app *appbase.BaseApp) {
		app.SetStoreLoader(appbase.UpgradeableStoreLoader(upgradeInfoPath))
	}
}
...
func TestSetLoader(t *testing.T) {
	...
	cases := map[string]struct {
		setLoader    func(*appbase.BaseApp)
		origStoreKey string
		loadStoreKey string
	}{
		"don't set loader": {
	...
	for name, tc := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			opts := []func(*appbase.BaseApp){appbase.SetPruning(store.PruneNothing)}
			if tc.setLoader != nil {
				opts = append(opts, tc.setLoader)
			}
			app := appbase.NewBaseApp(t.Name(), defaultLogger(), db, nil, opts...)
			capKey := sdk.NewKVStoreKey(appbase.MainStoreKey)
			app.MountStores(capKey)
...
func TestAppVersionSetterGetter(t *testing.T) {
	logger := defaultLogger()
	pruningOpt := appbase.SetPruning(store.PruneSyncable)
	db := dbm.NewMemDB()
	name := t.Name()
	app := appbase.NewBaseApp(name, logger, db, nil, pruningOpt)
...
func TestLoadVersionPruning(t *testing.T) {
	...
	pruningOpt := appbase.SetPruning(pruningOptions)
	db := dbm.NewMemDB()
	name := t.Name()
	app := appbase.NewBaseApp(name, logger, db, nil, pruningOpt)
...
func testLoadVersionHelper(t *testing.T, app *appbase.BaseApp, expectedHeight int64, expectedID sdk.CommitID) {
	lastHeight := app.LastBlockHeight()
	lastID := app.LastCommitID()
...
func TestOptionFunction(t *testing.T) {
	//logger := defaultLogger()
	//db := dbm.NewMemDB()
	//_ := appbase.NewBaseApp("starting name", logger, db, nil, testChangeNameHelper("new name"))
	//require.Equal(t, bap.name, "new name", "BaseApp should have had name changed via option function")
}

func testChangeNameHelper(name string) func(*appbase.BaseApp) {
	return func(bap *appbase.BaseApp) {
		//bap.name = name
	}
}
...
func TestBaseAppOptionSeal(t *testing.T) {
	...
	require.Panics(t, func() {
		app.SetRouter(appbase.NewRouter())
	})
}

func TestSetMinGasPrices(t *testing.T) {
	//minGasPrices := sdk.DecCoins{sdk.NewInt64DecCoin("stake", 5000)}
	//app := newBaseApp(t.Name(),  appbase.SetMinGasPrices(minGasPrices.String()))
...
func TestInitChainer(t *testing.T) {
	...
	db := dbm.NewMemDB()
	logger := defaultLogger()
	app := appbase.NewBaseApp(name, logger, db, nil)
	capKey := sdk.NewKVStoreKey(appbase.MainStoreKey)
	capKey2 := sdk.NewKVStoreKey("key2")
...
func TestCheckTx(t *testing.T) {
	...
	anteOpt := func(bapp *appbase.BaseApp) { bapp.SetAnteHandler(anteHandlerTxTest(t, capKey1, counterKey)) }
	routerOpt := func(bapp *appbase.BaseApp) {
...
func TestDeliverTx(t *testing.T) {
	...
	anteOpt := func(bapp *appbase.BaseApp) { bapp.SetAnteHandler(anteHandlerTxTest(t, capKey1, anteKey)) }
...
func TestSimulateTx(t *testing.T) {
	...
	anteOpt := func(bapp *appbase.BaseApp) {
...
func TestP2PQuery(t *testing.T) {
	addrPeerFilterOpt := func(bapp *appbase.BaseApp) {
	...
	idPeerFilterOpt := func(bapp *appbase.BaseApp) {
