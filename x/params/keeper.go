...
func NewKeeper(cdc *codec.Codec, key *sdk.KVStoreKey, tkey *sdk.TransientStoreKey, codespace did.CodespaceType) (
	k Keeper) {
	k = Keeper{
		Keeper: sdkparams.NewKeeper(cdc, key, tkey),
	}
	k.cdc = cdc
	k.paramSpace = k.Keeper.Subspace(DefaultParamspace).WithKeyTable(types.ParamKeyTable())
	return k
}
