...
func NewQuerier(keeper Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		default:
			return nil,
				app.UnknownRequest("unknown token name query endpoint")
		}
	}
}
*/
...
func queryOrder(
	...
	if len(path) == 0 {
		return nil,
			app.UnknownRequest("must specify the order id")
	}
	id, err := strconv.ParseInt(path[0], 10, 64)
	if err != nil {
		return nil,
			app.UnknownRequest(fmt.Sprintf("wrong format for requestid %s", err.Error()))
	}
