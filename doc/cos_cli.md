...
`dpcli`是一个工具，使您能够与 Darkpool Hub 网络中的节点进行交互，无论您是否自己运行它。 让我们恰当的设置它。 要安装它，请按照[安装步骤](./ installation.md)进行安装。
...
```bash
dpcli config node <host>:<port

# example: dpcli config node https://77.87.106.33:26657
```
...
```bash
dpcli config chain-id darkpool-x02
```
...
+ `dx0` 
	+ 从通过`dpcli keys add`生成的账户私钥中产生
	+ 用于接收资金
	+ 例如 `dx015h6vd5f0wqps26zjlwrc6chah08ryu4hzzdwhc`
+ `dx0valoper`
	+ 用于关联一个验证人和其操作者
	+ 用于发起staking操作命令
	+ 例如 `dx0valoper1carzvgq3e6y3z5kz5y6gxp3wpy3qdrv928vyah`
+ `dx0pub`
	+ 从通过`dpcli keys add`生成的账户私钥中产生
	+ 例如 `dx0pub1zcjduc3q7fu03jnlu2xpl75s2nkt7krm6grh4cc5aqth73v0zwmea25wj2hsqhlqzm`
+ `dx0valconspub`
	+ 在使用`dpd init`创建节点时生成
	+ 使用`dpd tendermint show-validator`获得该值
	+ 例如 `dx0valconspub1zcjduepq0ms2738680y72v44tfyqm3c9ppduku8fs6sr73fx7m666sjztznqzp2emf`
...
```bash
dpcli keys add <account_name>
```

接下来，你必须创建一个密码来保护磁盘上的密钥。上述命令的输出将包含种子短语。建议将种子短语保存在安全的地方，以便在忘记密码的情况下，最终可以使用以下命令从种子短语重新生成密钥：

```bash
dpcli keys add --recover
```

如果你检查你的私钥，你会看到`<account_name>` :

```bash
dpcli keys show <account_name>
```

通过下面的命令查看验证人操作者的地址：

```bash
dpcli keys show <account_name> --bech=val
```
...
```bash
dpcli keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
dpcli keys add --multisig=foo,bar,baz --multisig-threshold=2 multisig_address
dpcli keys add --multisig=baz,foo,bar --multisig-threshold=2 multisig_address
```

多签地址也可以在运行中生成并通过以下命令打印：

```bash
dpcli keys show --multisig-threshold K name1 name2 name3 [...]
```
...
在广播交易时，`dpcli`接受`--broadcast-mode`标识。 这个标识的值可以是`sync`（默认值）、`async`或`block`，其中`sync`使客户端返回 CheckTx 响应，`async`使客户端立即返回，而`block`使得 客户端等待 tx 被提交（或超时）。
...
```bash
dpcli tx send ... --fees=50000udap
```

或：

```bash
dpcli tx send ... --gas-prices=0.025udap
```
...
获取token的最佳方式是通过[Darkpool测试网水龙头](https://faucetcosmos.network)。如果水龙头对你不生效，尝试在[#darkpool-validator](https://riot.im/app/#/room/#cosmos-validators:matrix.org)上向人索要。水龙头需要你打算用于抵押股权的`dx0`开头的地址。
...
```bash
dpcli query account <account_darkpool>
```

::: warning Note
当你查询余额为零的帐户时，你将收到以下错误：`No account with address <account_darkpool> was found in the state.` 如果你在节点与区块链完全同步之前就查询，也会发生这种情况。这些都很正常。
:::
...
```bash
dpcli tx send <destination_darkpool> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> 
```
...
```bash
dpcli query account <account_darkpool>
dpcli query account <destination_darkpool>
```

你还可以使用`--block`标识查询在特定高度区块下你的余额：

```bash
dpcli query account <account_darkpool> --block=<block_height>
```

你可以通过在命令行中附加`--dry-run`标识来模拟交易而不实际广播它：

```bash
dpcli tx send <destination_darkpoolaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> \
  --dry-run
```
...
```bash
dpcli tx sign --validate-signatures signedSendTx.json
```

你可以将由JSON文件提供的已签名的交易广播至指定节点：

```bash
dpcli tx broadcast --node=<node> signedSendTx.json
```
...
```bash
dpcli query txs --events='message.sender=dx01...'
```

使用多个`标签`:

```bash
dpcli query txs --events='message.sender=dx01...&message.action=withdraw_delegator_reward'
```

通过`page`和`limit`来实现分页:

```bash
dpcli query txs --events='message.sender=dx01...' --page=1 --limit=20
```
...
```bash
dpcli tx slashing unjail --from <validator-operator-addr>
```
...
```bash
dpcli query slashing signing-info <validator-pubkey>
```
...
一旦主网上线，你可以把`dap`委托给一个验证人。这些委托人可以收到部分验证人的收益。阅读[Cosmos Token Model](https://github.com/cosmos/cosmos/raw/master/Cosmos_Token_Model.pdf)了解更多信息。
...
```bash
dpcli query staking validators
```

如果你想要获得单个验证人的信息，你可以使用下面的命令：

```bash
dpcli query staking validator <account_cosmosval>
```

#### 绑定 Token

在Cosmos Hub主网中，我们绑定`udap`，`1dap = 1000udap`。你可以把token绑定在一个测试网验证人节点上（即委托）：

```bash
dpcli tx staking delegate \
  --amount=10000000udap \
  --validator=<validator> \
  --from=<key_name> \
  --chain-id=<chain_id>
...
```bash
dpcli keys show [name] --bech val
```

其中`[name]`是初始化`dpd`时指定的键的名称。
...
```bash
dpcli query staking delegation <delegator_addr> <validator_addr>
```

或者你想查看所有当前的委托：

```bash
dpcli query staking delegations <delegator_addr>
```
...
```bash
dpcli query staking unbonding-delegation <delegator_addr> <validator_addr>
```

或者你可以查看当前你所有的unbonding-delegation:

```bash
dpcli query staking unbonding-delegations <account_cosmos>
```

此外，你可以从特定验证人获取所有unbonding-delegation：

```bash
dpcli query staking unbonding-delegations-from <account_cosmosval>
```
...
```bash
dpcli query staking redelegation <delegator_addr> <src_val_addr> <dst_val_addr>
```

或者，如果你可以检查所有当前的unbonding-delegation：

```bash
dpcli query staking redelegations <account_cosmos>
```

此外，你可以查询某个特定验证人的所有转出的重新绑定：

```bash
dpcli query staking redelegations-from <account_cosmosval>
```
...
```bash
dpcli query delegations-to <account_cosmosval>
```

### 治理

治理是Darkpool Hub的用户可以就软件升级，主网的参数或自定义文本提案并达成共识的过程。这是通过对提案进行投票来完成的，提案将由主要网络上的`dap`持有者提交。

关于投票过程的一些考虑因素：
+ 投票由绑定`dap`的持有者以1个绑定的`dap`对应1票方式投出
+ 委托人不投票的话会将票权继承给其验证人
...
```bash
dpcli tx gov submit-proposal param-change <path/to/proposal.json> \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
dpcli query gov proposal <proposal_id>
```

或者查询所有的有效提案：

```bash
dpcli query gov proposals
```
...
```bash
dpcli query gov proposer <proposal_id>
```
...
```bash
dpcli tx gov deposit <proposal_id> "10000000udap" \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
dpcli query gov deposits <proposal_id>
```

你还可以查询特定地址提交的存入金：

```bash
dpcli query gov deposit <proposal_id> <depositor_address>
```

#### 投票给一个提案

在提案的存入金达到`MinDeposit`后，投票期将开放。抵押了`dap`的持有人可以投票：

```bash
dpcli tx gov vote <proposal_id> <Yes/No/NoWithVeto/Abstain> \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
dpcli query gov vote <proposal_id> <voter_address>
```

你还可以查询提交给所有此前投给指定提案的投票：

```bash
dpcli query gov votes <proposal_id>
```
...
```bash
dpcli query gov tally <proposal_id>
```
...
```bash
dpcli query gov params
```

查询运行的治理参数的子集：

```bash
dpcli query gov param voting
dpcli query gov param tallying
dpcli query gov param deposit
```
...
```bash
dpcli query distribution params
```
...
```bash
dpcli query distribution outstanding-rewards
```
...
```bash
dpcli query distribution commission <validator_address>
```
...
```bash
dpcli query distribution slashes <validator_address> <start_height> <end_height>
```
...
```bash
dpcli query distribution rewards <delegator_address> <validator_address>
```
...
```bash
dpcli query distribution rewards <delegator_address>
```
...
dpcli keys add \
  p2 \
  --pubkey=dx0pub1addwnpepqtd28uwa0yxtwal5223qqr5aqf5y57tc7kk7z8qd4zplrdlk5ez5kdnlrj4

dpcli keys add \
  p3 \
  --pubkey=dx0pub1addwnpepqgj04jpm9wrdml5qnss9kjxkmxzywuklnkj0g3a3f8l5wx9z4ennz84ym5t
...
```bash
dpcli keys show --address p1p2p3
```

您还可以通过查看 key 的 JSON 输出或增加`--show-multisig`标识来查看multisig阈值，pubkey构成和相应的权重：

```bash
dpcli keys show p1p2p3 -o json

dpcli keys show p1p2p3 --show-multisig
```

创建多签交易的第一步是使用上面创建的多签地址初始化：

```bash
dpcli tx send dx01570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 10000000udap \
...
```bash
dpcli tx broadcast signedTx.json
```

## shell 自动补全脚本

可以通过完全命令生成主流的UNIX shell解释器（如`Bash`和`Zsh`）的`completion`命令，该命令可用于`dpd`和`dpcli`。

如果要生成Bash完成脚本，请运行以下命令：

```bash
dpd completion > dpd_completion
dpcli completion > dpcli_completion
```

如果要生成Zsh完成脚本，请运行以下命令：

```bash
dpd completion --zsh > dpd_completion
dpcli completion --zsh > dpcli_completion
```
...
```bash
echo '. dpd_completion' >> ~/.bashrc
echo '. dpcli_completion' >> ~/.bashrc
```
